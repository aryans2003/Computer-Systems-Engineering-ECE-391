...
void clear(void) {
    ...
    /*Setting screen to the top and updating the cursor location*/
    screen_x = 0;
    screen_y = 0;
...
void putc(uint8_t c) {
    ...
    if(c == '\n' || c == '\r') {
        screen_x = 0;
        if(screen_y != NUM_ROWS-1) { //When we havent reached the bottom of the screen
            screen_y++;
        } else {    //When we have reached the bottom of the screen
